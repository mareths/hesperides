...
public class PropertiesController extends AbstractController {
    ...
    public ResponseEntity<DetailedPropertiesPlatformOutput> getDetailedProperties(Authentication authentication,
                                                                                  @PathVariable("application_name") final String applicationName,
                                                                                  @PathVariable("platform_name") final String platformName,
                                                                                  @RequestParam(value = "properties_path", required = false) final String propertiesPath) {
        Platform.Key platformKey = new Platform.Key(applicationName, platformName);
        User user = new User(authentication);
        DetailedPropertiesPlatformView detailedPropertiesPlatformView = platformUseCases.getDetailedProperties(platformKey, propertiesPath, user);
        DetailedPropertiesPlatformOutput detailedPropertiesPlatformOutput = new DetailedPropertiesPlatformOutput(detailedPropertiesPlatformView);
        return ResponseEntity.ok(detailedPropertiesPlatformOutput);
    }
}
